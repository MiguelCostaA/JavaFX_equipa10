...
public class UtilizadoresClient extends Application {
	
	private static URI getBaseURI() {
		return UriBuilder.fromUri("http://localhost:8080/RESTServer/utilizador/").build();
	}
	...
	public void start(Stage primaryStage) throws Exception{
		...
		TableColumn<Utilizador, String> column1 = new TableColumn<>("Utilizador Nome");
		column1.setMinWidth(200);
		column1.setCellValueFactory(new PropertyValueFactory<>("nome"));
		
		TableColumn<Utilizador, String> column2 = new TableColumn<>("Utilizador Email");
		column1.setMinWidth(200);
		column2.setCellValueFactory(new PropertyValueFactory<>("email"));
		
		TableColumn<Utilizador, String> column3 = new TableColumn<>("Utilizador Password");
		column1.setMinWidth(200);
		column3.setCellValueFactory(new PropertyValueFactory<>("password"));
		
		TableColumn<Utilizador, String> column4 = new TableColumn<>("Utilizador Tarefas");
		column1.setMinWidth(200);
		column4.setCellValueFactory(new PropertyValueFactory<>("tarefas"));
        ...
        TableView.TableViewSelectionModel<Utilizador> selectionModel = tableView.getSelectionModel();
        
        selectionModel.setSelectionMode(SelectionMode.SINGLE);
        
        ObservableList<Utilizador> selectedItems = selectionModel.getSelectedItems();
		
        selectedItems.addListener(new ListChangeListener<Utilizador>() {
            @Override
            public void onChanged(Change<? extends Utilizador> change) {
		...
		btnNew.setOnAction(ae -> { 
			System.out.println("New utilizador data... ");
			showAddUpdateUtilizadorStage(primaryStage, new Utilizador()); 
			fillTableView(tableView);
        });
		
		btnEdit.setOnAction(ae -> {
			System.out.println("Editing data... ");			
			showAddUpdateUtilizadorStage(primaryStage, selectedItems.get(0));
			fillTableView(tableView);
		});
		...
		primaryStage.setTitle("Simple Utilizadores CRUD example");
	...
	private void showAddUpdateTarefaStage(Stage primaryStage, Tarefa tarefa, Utilizador utilizador) {
	...
	private void saveTarefa(Tarefa tarefa, Utilizador utilizador) {
		utilizador.getTarefas().add(tarefa);
	}
	...
	private List<Utilizador> getUtilizadores() {

		// Get the Books
		Gson gson = new Gson();

		String responseTarefasList = service.path("getUtilizadores")
				.request(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON)
				.get(String.class);

		List<Utilizador> utilizadores = Arrays.asList(gson.fromJson(responseTarefasList, Utilizador[].class));
				
		return utilizadores;
	}
	
	private void showAddUpdateUtilizadorStage(Stage primaryStage, Utilizador utilizador) {
		
		Stage stage = new Stage();
		
		stage.setTitle("Adicionar ou atualizar Utilizadores - in Modal Mode");
        stage.setX(300);
        stage.setY(300);
        stage.setWidth(300);
        stage.setHeight(300);
        ...
        root.add(lblNome, 0, 0, 1, 1);
        
        TextField txtNome = new TextField();
        lblNome.setId("lblNome");
        root.add(txtNome, 1, 0, 1, 1);
		...
		root.add(btnNovaTarefa, 2, 0, 1, 1);
		
		btnNovaTarefa.setOnAction(ae ->{
			System.out.println("New tarefa data... ");
			showAddUpdateTarefaStage(primaryStage, new Tarefa(), utilizador);
			//fillTableView(tableView);	
		});
		...
		btnSave.setOnAction(ae -> {
			System.out.println("A guardar... " + utilizador);
	...
	private void saveData(Utilizador utilizador) {
		try {
			
			Response response;
			String message = "Utilizador adicionado com sucesso";
			
			if(utilizador.getNome() != null) {
				message = "Utilizador atualizado com sucesso";
				response = service.path("updateUtilizador").request(MediaType.APPLICATION_JSON)
						.put(Entity.entity(utilizador, MediaType.APPLICATION_JSON), Response.class);	
			} else {
				message = "Utilizador adiconado com sucesso";
				response = service.path("addUtilizador").request(MediaType.APPLICATION_JSON)
						.post(Entity.entity(utilizador, MediaType.APPLICATION_JSON), Response.class);	
			}
			...
			showMessage("Erro ao guardar o utilizador", AlertType.ERROR);
	...
	private void showMessage(String message, AlertType alertType) {
        ...
        alert.setTitle("Utilizadores Application");
	...
	private boolean showConfirmationDialog(String confirmationMessage) {
		...
		alert.setTitle("Delete utilizador warning");
